...
class PatrolNode(BasicNavigator):
    ...
    def nav_to_pose(self, target_nose):
        """
        导航到指定位姿
        """
        self.waitUntilNav2Active()
        result = self.goToPose(target_nose)
    ...
    def get_current_pose(self):
        ...
        while rclpy.ok():
            try:
                ...
                self.get_logger().info(f'平移: {transform.translation}, 旋转四元数:\
                    {transform.rotation}, 旋转欧拉角: {rotation_euler}')
                return transform
    ...
    def record_image(self):
        ...
        if self.latest_image is not None:
            ...
            cv2.imwrite(f'{self.image_save_path}image_{pose.translation.x:3.2f}_{pose.translation.y:3.2f}.png'
                        ,cv_image)
